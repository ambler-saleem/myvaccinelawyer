...
Plugin URI: http://www.bestiaweb.com/wpdoctor/
...
Copyright 2015  BestiaWeb S.C.P.
...
<div class="wpdoctor">
	<h1><img src="<?php echo plugins_url( 'wpdoctor.png', __FILE__ ); ?>"><span><?php _e("WP DOCTOR", "wpdoctor"); ?> by Bestiaweb.com</span><a href="http://www.bestiaweb.com" target="_blank" title="Design web"><img src="<?php echo plugins_url( 'bestiaweb.png', __FILE__ ); ?>"></a></h1>
...
<strong><?php echo __("Last scan: ", "wpdoctor").' '.$myrows[0]->op1; ?></strong><br/>
	<h2><?php _e("Settings", "wpdoctor"); ?></h2>


		<form method="post" action="">
				<label><?php echo _e("Scanning period(in hours):", "wpdoctor"); ?></label>
	<input type="text" value="<?php echo esc_attr($myrows[0]->emailsend); ?>" id="emailsend" name="emailsend">
...
<h2><?php _e("Operations", "wpdoctor"); ?></h2>

	<a href="options-general.php?page=<?php echo $_GET["page"]; ?>&scan=1&nonce=<?php echo $nonce; ?>" class="button-primary"><?php _e("WP DOCTOR", "wpdoctor"); ?></a> <strong><?php _e("Scan malware, .htaccess file, file permissions, ... . This function is secure and not changes files.", "wpdoctor"); ?></strong>
<br/><br/><br/>
...
<a href="options-general.php?page=<?php echo $_GET["page"]; ?>&scan=2&nonce=<?php echo $nonce; ?>" class="button-primary"><?php _e("Clean Malware", "wpdoctor"); ?></a> <strong><?php _e("¡It is recommended that you scan and back up files before cleaning the files!", "wpdoctor"); ?></strong>
	...
	if($info_wpdoctor!="") echo '<br/><br/><h2 class="orange">'.__("RESULTS", "wpdoctor").'</h2>';

	echo $info_wpdoctor;

	echo '<hr/><h2>Not suspicious files</h2><form method="post" action=""><table>';
	...
	if($cont==1) echo '<tr><td>'.__("Not files", "wpdoctor").'</td></tr>';
	...
	echo '<hr/><h2>'.__("BLOCK IP", "wpdoctor").'</h2><form method="post" action=""><table>';
...
class WpDoctorMalCodeScan {
	...
	foreach($myrows as $r) {
		...
		if($option==1) {
			...
			$info_wpdoctor.='<h3>'.__("Domain in blacklist?", "wpdoctor").'</h3>';
		...
		if(is_file($dir.'.htaccess')) {
			...
			$info_wpdoctor.='<h3>'.__("Write in .htaccess file", "wpdoctor").'</h3>';
...
# BEGIN WordPress</pre>
		...
		if($option==1) { 
			...
			$info_wpdoctor.='<br/><h3>'.__("Malware scan results", "wpdoctor").'</h3><strong>'.count($this->scanned_files).' '.__("Files scaned", "wpdoctor").', '.count($this->infected_files).' '.__("suspicious files", "wpdoctor").'</strong><br/><form method="post" action=""><table>';
			...
			if(count($this->notpermdire)>0) $info_wpdoctor.='<h3>'.__("Changing directory permissions to 755", "wpdoctor").'</h3>';
			...
			if(count($this->notpermfiles)>0) $info_wpdoctor.='<h3>'.__("Changing file permissions to 644", "wpdoctor").'</h3>';
	...
	function scan($dir, $clean, $option) {
		...
		foreach($files as $file) {
			...
			if(is_file($dir.'/'.$file) && !in_array($dir.'/'.$file,$this->scanned_files)) {
				...
				$this->check(file_get_contents($dir.'/'.$file),$dir.'/'.$file, $clean);
